...
public class Game {
    ...
    protected Game(int WIDTH_SIZE, int HEIGHT_SIZE, int MAX_BOMBS, int COLOR_BOMBS) {
        ...
        matrixBomb.initMatrixBomb(MAX_BOMBS, COLOR_BOMBS);
        setStartCount();
        setEmptyFieldMatrix();
    }

    protected void clickRightButton(int x, int y) {
        if (matrixField.getFieldMatrix(x, y) == StatusUpperField.FLAG) {
            matrixField.setFieldMatrix(x, y, StatusUpperField.CLOSED);
            setCountOfBombs(1);
            return;
        }
        if (matrixField.getFieldMatrix(x, y) == StatusUpperField.CLOSED) {
            matrixField.setFieldMatrix(x, y, StatusUpperField.FLAG);
            setCountOfBombs(-1);
            return;
        }
    }
    ...
    protected void clickLeftButton(int x, int y) {
        if (matrixField.getFieldMatrix(x, y) == StatusUpperField.CLOSED) {
            if (matrixField.getField(x, y) == 0 && matrixBomb.getBombMatrix(x, y) == 0) {
                openArroundBlocks(x, y);
            }
            matrixField.setFieldMatrix(x, y,StatusUpperField.OPENED);
        }
        if (matrixField.getFieldMatrix(x, y) == StatusUpperField.OPENED && matrixField.getField(x, y) > 0 && checkAroundNumber(x, y)) {
            openArroundBlocks(x, y);
        }
        chekGameOver(x, y);
        checkGameWin();
    ...
    private boolean checkAroundNumber(int x, int y) {
        ...
        for (int xx = x - 1; xx < x + 2; xx++) {
            for (int yy = y - 1; yy < y + 2; yy++) {
                if (xx >= 0 && xx < WIDTH_SIZE && yy >= 0 && yy < HEIGHT_SIZE &&
                        matrixField.getFieldMatrix(xx, yy) == StatusUpperField.FLAG) {
                    if (xx == x && yy == y) {
                        continue;
                    }
                    count++;
        ...
        return matrixField.getField(x, y) == count;
    }
        ...
        for (int xx = x - 1; xx < x + 2; xx++) {
            for (int yy = y - 1; yy < y + 2; yy++) {
                if (xx >= 0 && xx < WIDTH_SIZE && yy >= 0 && yy < HEIGHT_SIZE &&
                    ...
                    matrixField.setFieldMatrix(xx, yy, StatusUpperField.OPENED);
                    chekGameOver(xx, yy);
                    if (matrixField.getField(xx, yy) == 0 && matrixBomb.getBombMatrix(xx, yy) == 0) {
    ...
    private void setEmptyFieldMatrix() {
        for (int x = 0; x < WIDTH_SIZE; x++) {
            for (int y = 0; y < HEIGHT_SIZE; y++) {
                ...
                matrixField.setField(x, y, count);
            }
        }
    }
    ...
    protected int getOpenedField(int x, int y) {
        return matrixField.getField(x, y);
    }
