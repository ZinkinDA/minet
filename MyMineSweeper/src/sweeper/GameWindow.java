...
public class GameWindow extends JFrame {
    ...
    public GameWindow(final int WIDTH_SIZE, final int HEIGHT_SIZE, final int SIZE_BLOCK, final int MAX_BOMBS, final String TITLE) {
        ...
        this.TITLE = TITLE;
        game = new Game(WIDTH_SIZE, HEIGHT_SIZE, MAX_BOMBS, COLOR_BOMBS.length);
        initLabel();
        initGameField();
        initFrame();
    }
    ...
    private void changeLabel() {
        if (!game.ifGameOver()) {
            label.setText("Осталось мин: " + game.getCountOfBombs() + "  Осталось открыть: " + (game.getVolumeBoxOnField() - game.getOpenBoxOnTheField() - MAX_BOMBS));
        }
        if (game.ifGameOver()) {
            label.setText("Game Over!");
        }
        if (!game.ifGameOver() && game.checkGameWin()) {
    ...
    private void initGameField() {
        ...
        panelField.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                if (e.getButton() == 3 && !game.ifGameOver() && !game.checkGameWin()) {
                    game.clickRightButton(x, y);
                }
                if (e.getButton() == 1 && !game.ifGameOver() && !game.checkGameWin()) {
                    game.clickLeftButton(x, y);
                }
                if (e.getButton() == 2) {
                    game.clickCenterButton(x, y);
                }
                changeLabel();
                panelField.repaint();
            }
    ...
    private class CanvasGameField extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (int x = 0; x < WIDTH_SIZE; x++) {
                for (int y = 0; y < HEIGHT_SIZE; y++) {
                    if (game.getFieldMatrix(x, y) != StatusUpperField.OPENED) {
                        paintCloseBox(x, y, g);
                        if (game.getFieldMatrix(x, y) == StatusUpperField.FLAG) {
                            paintFlagOnField(x, y, g);
                        }
                    }
                    if (game.getFieldMatrix(x, y) == StatusUpperField.OPENED) {
                        paintOpenBox(x, y, g);
                        if (game.getBombMatrix(x, y) > 0) {
                            paintBombOnField(x, y, 0, g);
                            paintExplodedBombOnField(x, y, g);
                        }
                        if (game.getOpenedField(x, y) > 0) {
        ...
        private void paintNumberOnField(int x, int y, Graphics g) {
            g.setColor(COLOR_NUMBERS[game.getOpenedField(x, y) - 1]);
            g.drawString(String.valueOf(game.getOpenedField(x, y)),
